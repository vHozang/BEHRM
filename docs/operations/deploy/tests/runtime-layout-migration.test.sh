...
env_validation_line="$(grep -n 'if \[ ! -f "$BACKEND_DIR/.env" \]' "$deploy_script" | cut -d: -f1)"
nginx_start_line="$(grep -n '^docker compose up -d php reverb nginx$' "$deploy_script" | cut -d: -f1)"
test -n "$build_line" && test -n "$root_migration_line" && test -n "$env_validation_line" && test -n "$nginx_start_line"
test "$build_line" -lt "$root_migration_line"
test "$root_migration_line" -lt "$env_validation_line"
test "$env_validation_line" -lt "$nginx_start_line"
grep -q 'MIGRATION_TARGET_UID' "$deploy_script"
grep -q 'MIGRATION_TARGET_GID' "$deploy_script"
