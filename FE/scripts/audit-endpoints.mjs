...
const backend = join(workspace, 'BE');
const manifestPath = join(workspace, 'docs', 'api', 'endpoint-manifest.json');
...
const routeList = () => {
  ...
  for (const args of [
    ['compose', 'exec', '-T', 'php', 'php', 'artisan', 'route:list', '--json'],
    ['compose', 'run', '--rm', '--no-deps', 'php', 'php', 'artisan', 'route:list', '--json'],
  ]) {
    try {
      return parse(execFileSync('docker', args, {
        cwd: backend,
        encoding: 'utf8',
        stdio: ['ignore', 'pipe', 'pipe'],
      }));
  ...
  throw new Error(`Unable to read Laravel routes: ${localError?.message || 'PHP and Docker are unavailable'}`);
};
...
const frontendCalls = () => {
  const files = walk(join(frontend, 'src')).filter((path) => ['.js', '.vue', '.mjs'].includes(extname(path)));
  const calls = [];
  const usedResources = new Set();
  for (const file of files) {
    const source = readFileSync(file, 'utf8');
    const label = relative(frontend, file).replaceAll('\\', '/');
...
if (process.argv.includes('--write')) {
  ...
  console.log(`Wrote ${relative(workspace, manifestPath)} with ${routes.length} routes and ${resources.length} generic resources.`);
  process.exit(0);
}

if (!existsSync(manifestPath)) {
  console.error(`Missing endpoint manifest: ${relative(workspace, manifestPath)}`);
  process.exit(1);
}
