...
const service = await readFile('src/services/organizationChartService.js', 'utf8');
assert.ok(service.includes('/organization-chart/structure'), 'Service chưa gọi endpoint sơ đồ đơn vị');

const chart = await readFile('src/views/OrganizationChart.vue', 'utf8');
for (const marker of [
  'Toàn công ty',
  'Chi nhánh',
  'Phòng ban',
...
const baseCard = await readFile('src/components/BaseCard.vue', 'utf8');
assert.ok(baseCard.includes('contentClass'), 'BaseCard chưa hỗ trợ tùy chỉnh lớp nội dung');

const node = await readFile('src/components/OrgTreeNode.vue', 'utf8');
