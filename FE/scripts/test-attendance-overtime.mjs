...
import { statusVariant, statusVN } from '../src/utils/approvalSteps.js';
...
const service = await readFile('src/services/attendanceService.js', 'utf8');
...
const attendance = await readFile('src/views/Attendance.vue', 'utf8');
for (const marker of [
...
const timesheet = await readFile('src/views/MonthlyTimesheet.vue', 'utf8');
for (const marker of [
...
const axiosClient = await readFile('src/services/axiosClient.js', 'utf8');
assert.match(
  axiosClient,
...
const attendanceStore = await readFile('src/stores/attendanceStore.js', 'utf8');
for (const marker of [
  'timesheetPageCache',
  'timesheetRequests',
  'timesheetCacheTtlMs',
...
const overtime = await readFile('src/views/OvertimeRequests.vue', 'utf8');
for (const marker of [
  'Đơn đăng ký',
  'Ticket được giao',
  'Máy thực tế',
...
const adjustments = await readFile('src/views/AttendanceAdjustments.vue', 'utf8');
...
const salaries = await readFile('src/views/Salaries.vue', 'utf8');
...
const settings = await readFile('src/views/Settings.vue', 'utf8');
